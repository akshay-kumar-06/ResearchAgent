...
async def create_research(request: ResearchRequest, background_tasks: BackgroundTasks):
    ...
    logger.info(
        "research_request_received", research_id=research_id, query=request.query
    )
...
async def execute_research(research_id: str, query: str):
    ...
    try:
        ...
        active_researches[research_id] = {
            ...
            "research_id": research_id,
            "result": result,
            "created_at": active_researches[research_id].get(
                "created_at", datetime.now()
            ),
        }
        ...
        logger.error(
            "research_background_failed", research_id=research_id, error=str(e)
        )
...
@app.get(
    "/research/{research_id}/status", response_model=ResearchStatus, tags=["Research"]
)
async def get_research_status(research_id: str):
    ...
    if research_id in active_researches:
        ...
        return ResearchStatus(
            ...
            or active.get("result", {}).get("error_message"),
...
async def get_metrics():
    ...
    Returns:
        Dictionary with various application metrics
    """
