...
from app.core.metrics import record_research_metrics
...
def create_research_graph():
    ...
    workflow.add_conditional_edges(
        "plan", should_continue, {"search": "search", END: END}
    )

    # Add conditional edges from search
    workflow.add_conditional_edges(
        "search", should_continue, {"analyze": "analyze", END: END}
    )

    # Add conditional edges from analyzer
    workflow.add_conditional_edges(
        "analyze", should_continue, {"write": "write", END: END}
    )
...
async def run_research(query: str) -> dict:
    ...
    try:
        ...
        record_research_metrics(
            ...
            sources_count=final_state.get("total_sources", 0),
        )
        ...
        execution_time = time.time() - start_time
        record_research_metrics(status="error", execution_time=execution_time)
        # Return error state matching ResearchState structure
        return {
            "original_query": query,
            "research_plan": None,
            "sub_questions": [],
